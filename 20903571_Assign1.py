...
Student Name: Inam ULHAQAZAD
Student ID: 20903571
Assignment #: 1
Student Email: iuaa@connect.ust.hk
Course Name: COMP 5212
...
def compute_softmax_loss(W, X, y, reg):
    ...
    logit = logit-mx #avoid instability
    
    #softmax probability
    softmx = np.exp(logit)/np.sum(np.exp(logit), axis=1, keepdims=True)
    
    loss1 = logit[np.arange(N), y]
    loss2 = -np.log(np.sum(np.exp(logit), axis=1, keepdims=True))
    
    #loss = -np.mean(loss1+loss2)
    
    loss = np.sum(loss1+loss2)
    loss /= -N
   ...
   # print (X.T.shape)
    ...
    # chain rule and regularization term
